...
<template>
	<div style="display: flex;flex-wrap: wrap;">
		<div class="captcha-tips" :style="{ color: tipsColor }">{{$t('SelectCaptchaInOrder')}}{{ imgData }}</div>
		<div style="display: flex;">
			<div class="captcha-img" :style="{ backgroundImage: 'url(' + imgUrl + ')' }">
				<div v-for="(item, index) in imgValues" :key="index" :class="['captcha-num', { 'captcha-clicked': item.isCheck }]" @click="click(index)"></div>
			</div>
			<div class="captcha-reset" @click="reset" :style="{ color: resetColor }"><i class="el-icon-refresh"></i></div>
		</div>
	</div>
</template>
