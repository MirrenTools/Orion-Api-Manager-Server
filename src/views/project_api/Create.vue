			...
			<el-form ref="apiForm" label-position="right" label-width="120px" :model="api" :rules="apiRules">
				...
				<el-form-item :label="$t('Parameters')">
					<div>
						...
						<div>
							<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 10 }" :placeholder="$t('EnterRequestBody')" v-model="api.body"></el-input>
						</div>
					</div>
				</el-form-item>
		...
		data() {
			return {
				...
				api: {
					...
					consumes: ['x-www-form-urlencoded', 'multipart/form-data', 'application/json', 'application/xml'],
					produces: [],
			...
			submitCreateApi() {
				this.$refs.apiForm.validate(valid => {
					if (valid) {
						...
						if (this.api.consumes.length > 0) {
							reqData.consumes = JSON.stringify(this.api.consumes);
						}
						if (this.api.produces.length > 0) {
