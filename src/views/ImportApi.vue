	...
	export default {
		...
		methods: {
			...
			loadDocumentOfSwagger(data) {
				...
				var orionData = this.convertOaiCommonToOrion(data);
				var servers = [];
				var nsurl = (data.host ? data.host : '') + (data.basePath ? data.basePath : '');
			...
			convertOaiCommonToOrion(data) {
				...
				if (data.info != null) {
					...
					if (info.license != null) {
						...
						orionData.description += '<div><a href="' + (info.license.url ? info.license.url : "#") +
							'" target="_blank">License ' + (info.license.name ? info.license.name : "") + '</a></div>';
					}
