<template>
	<div style="width: 98%; max-width: 1240px;margin: auto;padding-top: 15px;">
		<div style="margin-top: 15px;">
			<el-input placeholder="文档的URL,支持导入Orion-API-Gateway、OpenAPI 3.0与Swagger 2.0的JSON格式API文档,代理请求可加上P: 示例:P:https://xxx.xxx/xx" v-model="documentUrl">
		...
		<div style="text-align: center;margin:30px auto 50px ;"><el-button type="primary" @click="saveSubmit()" :loading="saving">确定导入</el-button></div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		loadDocumentOfOpenAPI(data) {
			...
			this.requestLoading = false;
			this.projectLoading = true;
			var orionData = this.convertOaiCommonToOrion(data);
			var servers = [];
		...
		/**
		 * 将OpenAPI与Swagger的通用属性转换为Orion的属性
		 * @param {Object} data
		 */
		convertOaiCommonToOrion(data) {
			var orionData = {};
			//项目信息转换开始
			if (data.info != null) {
				var info = data.info;
				...
				if (info.license != null) {
					...
					orionData.description += '<div><a href="' + (info.license.url || '#') + '" target="_blank">License ' + (info.license.name || '') + '</a></div>';
				}
