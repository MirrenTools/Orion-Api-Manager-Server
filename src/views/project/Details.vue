	...
	<div>
		...
		<div style="width: 98%; max-width: 1240px;margin: auto">
			...
			<table v-show="mode === 'view'" style="width: 100%;" cellspacing="10">
				...
				<tr>
					<td class="project-item">主机地址</td>
					<td>
						{{ project.host }}
						<span v-show="project.basePath">根路径 {{ project.basePath }}</span>
					</td>
				</tr>
				<tr>
					<td class="project-item">服务协议</td>
					<td>
						<span v-for="scheme in project.schemes" :key="scheme" style="margin-right: 5px;">{{ scheme }}</span>
					</td>
				</tr>
					...
					<el-collapse-item v-for="g in groups" :key="g.groupId">
						...
						<div style="padding-left: 10px;padding-bottom: 10px;">
							<div style="display: flex;">
								<el-button size="mini" type="primary" @click="groupMoveUp(g.groupId)">上移</el-button>
						...
						<div v-for="api in g.apis" :key="api.apiId" style="margin-bottom: 10px;">
							<div :class="['api', api.method]">
								...
								<div style="padding:5px 10px;text-align: right;">
									...
									<el-button size="mini" type="primary" @click="apiMoveUp(api.apiId)">上移</el-button>
									<el-button size="mini" type="primary" @click="apiMoveDown(api.apiId)">下移</el-button>
									<a :href="'#/index/get/project/api/'+project.key+'/'+api.groupId+'/'+api.apiId" target="_blank" style="margin-left: 10px;"><el-button size="mini" type="primary">查看详情</el-button></a>
								</div>
							</div>
						</div>
					</el-collapse-item>
	...
	data() {
		return {
			...
			editRules: {
				...
				schemes: [{ schemes: 'array', required: true, message: '请至少选择一个服务', trigger: 'change' }]
			},
			/**当前的模式*/
			mode: MODE_VIEW,
			/**是否显示新建分组的窗口*/
			...
			groupDataRules: {
				name: [{ required: true, message: '请输入分组的名称', trigger: 'blur' }],
				summary: [{ required: true, message: '请输入分组的简介', trigger: 'blur' }]
			},
			/**分组与接口合集*/
			groups: [],
			/**分组列表加载中*/
			...
			getProjectAPI(
				...
				resp => {
					...
					if (data.code == 200) {
						...
						this.project.schemes = JSON.parse(this.project.schemes);
